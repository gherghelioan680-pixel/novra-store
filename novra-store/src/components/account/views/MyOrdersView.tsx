  ...
  return (
    <div>
      ...
      <div className="rounded-xl border border-white/10 bg-novra-card/30">
        {loading ? (
          <p className="p-8 text-center text-sm text-gray-500">{t("loading")}</p>
...
function OrderDetailModal({ order, onClose }: { order: Order; onClose: () => void }) {
  ...
  const timeline: { status: OrderStatus; label: string }[] = [
    ...
    { status: "processing", label: t("statusProcessing") },
    { status: "shipped", label: t("statusShipped") },
    { status: "delivered", label: t("statusDelivered") },
  ];
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-4 sm:items-center">
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-white/10 bg-novra-card shadow-2xl">
        <div className="sticky top-0 flex items-center justify-between border-b border-white/10 bg-novra-card px-5 py-4">
          <h3 className="font-semibold text-white">{t("orderDetails")}</h3>
        ...
        <div className="space-y-5 p-5">
          ...
          {order.status === "cancelled" && (
            <p className="text-sm text-red-300">{t("orderCancelled")}</p>
          )}

          <div className="rounded-xl border border-white/8 bg-novra-bg/30 p-4">
            <p className="mb-2 text-[10px] uppercase tracking-widest text-gray-500">{t("products")}</p>
          ...
          <div className="rounded-xl border border-white/8 bg-novra-bg/30 p-4 text-sm">
            <div className="mb-2 flex items-center gap-2 text-gray-500">
              <MapPin size={14} />
              <span className="text-[10px] uppercase tracking-widest">{t("shippingAddress")}</span>
            </div>
          ...
          <div className="flex items-center gap-2 text-sm text-gray-400">
            <CreditCard size={14} />
            <span>
              {order.paymentMethod === "card" ? t("cardPayment") : t("cashOnDelivery")}
            </span>
          ...
          {order.awbTracking && (
            <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4 text-sm">
              <div className="mb-2 flex items-center gap-2 text-emerald-400">
                <Truck size={14} />
                <span className="text-[10px] uppercase tracking-widest">{t("tracking")}</span>
              </div>
